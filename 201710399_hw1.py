...
class Tokenizer:
    ...
    async def update_vocab(self, remove_special_chars: bool = True):
        ...
        train_data: List[List[str]] = self.preprocess(
            text=self.get_txt_data(self.train_file_path), remove_special_chars=remove_special_chars
        )
        iteration = 0
    ...
    def tokenize(self):
        ...
        progress = 0
        work = len(input_data)
        total_token_length = 0  # 전체 토큰 길이
        token_count = 0  # 전체 토큰 개수
        length_distribution = {}  # 길이별 분포
            ...
            while start < len(word_str):
                ...
                while end > start:
                    ...
                    if substr in self.vocab:
                        ...
                        token_count += 1
                        length_distribution[token_length] = length_distribution.get(token_length, 0) + 1
                        break
                    end -= 1
                ...
                if not found:
                    ...
                    total_token_length += 1
                    token_count += 1
                    length_distribution[1] = length_distribution.get(1, 0) + 1
